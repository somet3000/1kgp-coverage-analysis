...
write.table(genes, 'data/out/high_coverage/gene_info.txt', sep = '\t', row.names = FALSE)

snp.df <- read.table('data/temp/high_coverage/plink.bim', header = FALSE)
...
write.table(snp.df, 'data/out/high_coverage/snp_info.txt', sep = '\t', row.names = FALSE)
