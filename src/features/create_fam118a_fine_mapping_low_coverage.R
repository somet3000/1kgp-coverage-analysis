...
rnaseq.data <- read.table('data/out/low_coverage/matrix_eqtl_phenotype.txt')
...
write.table(fam118a.expr, 'data/out/low_coverage/fam118a_expression.txt')

# read in SNP info and gene info files
snp.info <- read.table('data/out/low_coverage/snp_info.txt', header=TRUE)
...
genotype.data <- read.table('data/out/low_coverage/matrix_eqtl_genotype_data.txt')
...
write.table(genotype.data, 'data/out/low_coverage/fam118a_genotype.txt')
