...
snp.data <- read.table('data/temp/low_coverage/plink.raw', header = TRUE, nrows = 500, colClasses = c('character', 'character', rep(NULL, 101231)))
snp.samples <- snp.data$IID
rnaseq.data <- rnaseq.data[, colnames(rnaseq.data) %in% snp.samples]

print('writing output!')
# save output to file
write.table(rnaseq.data, 'data/out/low_coverage/matrix_eqtl_phenotype.txt')
